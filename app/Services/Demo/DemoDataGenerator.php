...
/**
 * Builds realistic-looking sample data (2 sites, 2 sources, N days of snapshots)
 * for a given user so their dashboards/trends are populated before any real
 * EDR/XDR API is connected. Idempotent — re-running refreshes the sample data.
 */
class DemoDataGenerator
{
    public function __construct(private Summarizer $summarizer) {}

    /**
     * @return array{sites:int, sources:int, per_snapshot:int}
     */
    public function generateFor(User $user, int $days = 14): array
    {
        $london = Site::updateOrCreate(['user_id' => $user->id, 'name' => 'HQ — London']);
        $singapore = Site::updateOrCreate(['user_id' => $user->id, 'name' => 'Branch — Singapore']);

        $a = $this->seedSource($user, $london, 'Demo EDR — London', 'LDN', 120, $days);
        $b = $this->seedSource($user, $singapore, 'Demo EDR — Singapore', 'SGP', 80, $days);

        return ['sites' => 2, 'sources' => 2, 'per_snapshot' => $a + $b];
    }

    private function seedSource(User $user, Site $site, string $name, string $tag, int $count, int $days): int
    {
        ...
        // Idempotent: clear prior demo snapshots (FK cascade removes their endpoints).
        $source->snapshots()->delete();

        $fleet = $this->buildFleet($count, $tag);
        $lastSnapshotId = null;

        for ($d = $days - 1; $d >= 0; $d--) {
            $capturedAt = now()->subDays($d)->setTime(8, 0);

            $normalized = [];
            foreach ($fleet as $host) {
                $normalized[] = $this->endpointForDay($host, $capturedAt, $d);
            }
    ...
    private function endpointForDay(array $host, Carbon $capturedAt, int $day): array
    {
        ...
        if ($score > 25) {
            $lastSeen = $capturedAt->copy()->subMinutes(rand(1, 60 * 20));
            $health = 'online';
        } elseif ($score > 8) {
            $lastSeen = $capturedAt->copy()->subDays(rand(1, 6));
            $health = 'stale';
        } else {
            $lastSeen = $capturedAt->copy()->subDays(rand(8, 40));
            $health = 'offline';
        }

        $compliance = $health === 'offline'
            ? 'non_compliant'
